...
static void gran_nibble_info(uint8_t value, struct valinfo_s *info,
                             unsigned int nbits)
{
  const struct nibble_info_s *table = g_info_table[nbits];
  const struct nibble_info_s *nibinfo;
  uint8_t mask;
...
static void gran_info_combine(const struct valinfo_s *msinfo,
                              unsigned int msbits,
                              struct valinfo_s *lsinfo,
                              unsigned int lsbits)
{
...
static void gran_byte_info(uint8_t value, struct valinfo_s *info,
                           unsigned int nbits)
{
  uint8_t mask;
...
static void gran_hword_info(uint16_t value, struct valinfo_s *info,
                            unsigned int nbits)
{
  uint16_t mask;
...
void gran_info(struct mm_gran *gran, struct graninfo *info)
{
  uint32_t mask;
  uint32_t value;
  uint16_t mxfree;
  ...
  info->log2gran   = gran->log2gran;
  info->ngranules  = gran->ngranules;
  info->nfree      = 0;
  info->mxfree     = 0;
  mxfree           = 0;
  ...
  for (granidx = 0; granidx < gran->ngranules; granidx += 32)
    {
      ...
      gatidx = granidx >> 5;
      value  = gran->gat[gatidx];

      /* The final entry is a special case */

      if ((granidx + 32) > gran->ngranules)
        {
          nbits  = gran->ngranules - granidx;
          mask   = ((1ul << nbits) - 1);
          value  &= mask;
        }
      else
